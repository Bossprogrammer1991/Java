...
public class Mean_class {
    ...
    public static ArrayList<Integer> getMeanForAttClasses(ArrayList<int[]> vals, ArrayList<String> classes, int i) {
	ArrayList<Integer> means = new ArrayList<Integer>();
	int first_sum = 0;
	int numdata_first = 0;
	int second_sum = 0;
	int numdata_second = 0;
	for(int j = 0; j < vals.size(); j++)
		...
		else if((classes.get(j)).equals("1"))
		    {
			numdata_first++;
			first_sum += cur[i];
		    }
		else if((classes.get(j)).equals("2"))
		    {
			numdata_second++;
			second_sum += cur[i];
		    }
	...
	int mean1 = first_sum / numdata_first;
	means.add(mean1);
	int mean2 = second_sum / numdata_second;
	means.add(mean2);
	return means;
    }
 ...
 public static int getMeanForAtt(ArrayList<int[]> vals, int i) {
    ...
    public static void outputLines(Scanner s, ArrayList<int[]> vals, ArrayList<int[]> vals_train, ArrayList<String> classes, ArrayList<String> exs, BufferedReader br, BufferedWriter bw)  throws java.io.IOException{
		...
		for(int i = 0; i < 3; i++)
			...
			if(cur[i] == -1)
			    {
				if((classes.get(index)).equals("1"))
				    {
					ArrayList<Integer> class_means = getMeanForAttClasses(vals, classes, i);
					bw.write(class_means.get(0) + ",");
				    }
				else if((classes.get(index)).equals("2"))
				    {
					ArrayList<Integer> class_means = getMeanForAttClasses(vals, classes, i);
					bw.write(class_means.get(1) + ",");
				    }
				else{}
			    }
			else{
...
public static void outputTestLines(Scanner s, ArrayList<int[]> vals, ArrayList<int[]> vals_train, ArrayList<String> classes, ArrayList<String> exs, BufferedReader br, BufferedWriter bw)  throws java.io.IOException{
        String line;
        int numlines = 0;
        while((line = br.readLine()) != null) {
            numlines++;
	...
	outputTestLines(output_test_scan, vals_test, vals_train, classes_test, exs_test, br_out_test, bw_out_test);
	br_train.close();
	br_test.close();
	br_out_test.close();
	br_out_train.close();
